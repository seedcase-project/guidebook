...
use pull requests for several reasons:

1. Helps to maintain the code and text quality and minimise the risk of
   errors, typos, or other issues.
2. Enables effective collaboration by allowing changes to be reviewed
   and discussed.
3. Provides a structured process for integrating new developments into a
   project.
...
Doing it through VS Code, however, is a faster way, though it requires
some setting up and practice using it. To be able to create pull
...
Now, you can use the command palette (Ctrl+Shift+P or Cmd+Shift+P) and search for "GitHub: Create Pull Request" to create a pull request from the current branch you are working on. This will open a new pull request form in the sidebar where you can fill out the title, description, and other details of your PR before submitting it.
...
PRs are the highest priority tasks. Timely reviews keep the work moving
...
merge conflicts requires manual editing and resolving to continue.
...
- Use the GitHub interface to resolve the conflicts directly on the
  ...
  can't handle more complicated conflicts, so you need to resolve it on
