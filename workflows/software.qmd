...
*and then* write code that will pass those tests. However, this doesn't
always happen in exactly this order every time. It's also impossible to
know whether someone followed this order of writing tests first and then
code when receiving a pull request, unless you watched that person write
the tests and code as they wrote it. That's why for us, test-driven
development means that any pull request with changes to the codebase
that adds new or modifies existing behaviour always includes
comprehensive enough tests to show that the code actually has the
intended behaviour.

The only time when writing tests first is actually really effective is
when trying to resolve a bug. Start by writing a test that reproduces
the bug, so that you can more accurately identify what is needed to
trigger the bug. Then you fix the code so that the test of the bug no
longer fails.
