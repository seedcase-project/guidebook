# Changelog

Since we follow
[Conventional Commits](https://decisions.seedcase-project.org/why-conventional-commits/),
we're able to automatically create formal "releases" of the website
based on our commit messages. Releases in the context of websites are
simply snapshots in time of the website content. We use
[Commitizen](https://decisions.seedcase-project.org/why-semantic-release-with-commitizen/)
to automatically create these releases using
[SemVer](https://semverdoc.org) as the version numbering scheme.

Because releases are created based on commit messages, a new release is
created quite often---sometimes several times in a day. This also means
that any individual release will not have many changes within it. Below
is a list of the releases we've made so far, along with what was changed
within each release.

## 0.11.0 (2026-09-08)

### Feat

- ✨ move commits and PRs content from elsewhere (#157)

## 0.10.0 (2026-03-31)

### Feat

- ✨ section on test-driven development (#123)

## 0.9.1 (2026-03-30)

### Refactor

- ♻️ match iteration guide with what we currently do (#122)

## 0.9.0 (2026-03-24)

### Feat

- ✨ add section on Doc Driven Development (#108)

## 0.8.4 (2026-03-23)

### Refactor

- :recycle: clarify `docs` commit type for non-code projects (#118)

## 0.8.3 (2026-03-17)

### Refactor

- :recycle: clarify it is incremental and iterative (#121)

## 0.8.2 (2026-03-09)

### Fix

- :pencil2: correct link address to our GitHub App (#119)

## 0.8.1 (2026-03-03)

### Refactor

- :recycle: clarify to do several event storms before writing (#115)

## 0.8.0 (2026-03-03)

### Feat

- ✨ add note on moving in-progress tasks to next iteration (#113)

## 0.7.0 (2026-02-23)

### Feat

- ✨ pull requests chapter (#102)

## 0.6.0 (2026-02-18)

### Feat

- :wrench: add CONTRIBUTING and CHANGELOG to sidebar (#104)

## 0.5.0 (2026-02-12)

### Feat

- :sparkles: how-to guide on doing domain-driven design (#94)

## 0.4.0 (2026-02-09)

### Feat

- :sparkles: visual overview of software development workflow (#92)

## 0.3.0 (2025-12-09)

### Feat

- :sparkles: add content to the landing page (#78)

## 0.2.0 (2025-12-08)

### Feat

- ✨ add commit guidelines (#58)
- :sparkles: add "writing Python" guide section (#48)
- :sparkles: guide on tasks to do during iteration (#33)
- :sparkles: iteration chapter with description of it (#31)
- :sparkles: add link to `seedcase-project.org` in `body-header` (#52)
- :sparkles: add making Python test guide page (#39)
- :sparkles: end of iteration guide (#34)
- :sparkles: guide on start of iteration (#32)
- :sparkles: guide to make GitHub Apps to make tokens (#21)

### Fix

- :bug: add favicon directly in quarto config (#64)

### Refactor

- :recycle: move review of longer-term roadmap up in iteration start
  agenda (#63)
- ♻️ minor changes to Tests section (#51)
