...
process. This page will guide you through on making tests for your
...
and if its doing what you expect. And while you are completely free (and
...
internal code without having to also update tests for that internal
...
testing tools will be better able to find and run only those tests for
...
1.  Given: Set up the test data and any other necessary conditions. This
    step isn't always required, as it is used to create the test data
    (if needed) and any other extra conditions that are needed for the
    test to work. This might be where you create the `expected` output
    that you want to check against.
2.  When: Call the function or method being tested. This would be the
    `actual` output.
3.  Then: Check that output of the function or method (`actual`) is as
...
helpful---for reviewers and your future self---to include them. This is the same
reasoning for using docstrings for test functions. It can help
explain what the test is doing and why it is important.

In generally, it's best to keep the tests as simple as possible and only test one thing at a time.
...
[`template-python-package`](https://github.com/seedcase-project/template-python-package)
already has this option set up in the `pytest.ini` file, so you don't
