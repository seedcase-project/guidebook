...
needs manually fixes using Ruff and mypy, you can check the code with:
...
the code written inside of the public-facing functionality to be smaller
...
1.  An individual public-facing functionality has it's own Python file.
    Usually this means that each public function or class has its own
    file, but if the functionality is similar enough to others, it could
...
4.  If internal functionality is kept in the `internal/` folder, then
    group the internal functions by their common action into a single
    file.
...
`check_heart_rate()`. Both of these functions has an internal function
...
effects"](#side-effects) or conditionally changes it's action in
...
tends to produce more maintainable and more predictable code. The way
you write more functional code include:

-   Keeping functions or methods with clear input and one type of
    output. There is more explanation on this in the [type
...
-   Prefer using functionals like `map()`, `filter()`, and `reduce()`
    rather than using loops or list comprehensions, as they tend to be
...
capabilities are not as strong as some other languages, which can
...
(`True` or `False`). So if you were to look at the documentation for the
...
give an error. Unlike other programming languages, Python does not
enforce type hints when using the code as it is a [dynamically
...
Side effects are when the code does something that isn't obvious, isn't
clear from the name of the function or method, or if it does actions
that don't result in a `return`.
...
you want to avoid side effects as that introduces unpredictability in
your code.
...
As already described above in the [where to write
...
# Import the `is.py` script by using the relative `.` import syntax.
from .is import _is_data_object

# Use `__all__` to explicitly define what is exported from this module.
__all__ = [
    "_is_data_object",
]
```

That way, when you import the internal functionality in other scripts,
you can use either:
...
Without good documentation, code is near to unusable or at the least
extremely difficult to use. Documenting at the code level is done with
["docstrings"](https://peps.python.org/pep-0257/) in Python.

At the least, you should write a docstring for every public-facing
...
def check_heart_rate(data: dict) -> bool:
    ...
    Args:
        data (dict): The data object containing the "heart_rate"
            variable.

    Returns:
        bool: True if the heart rate is not normal, False otherwise.
