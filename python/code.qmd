...
This is the second resource you should use to learn about how to write
effective and well-styled Python code.
...
and consistent. This means that writing code in a more
[functional programming](https://en.wikipedia.org/wiki/Functional_programming)
style tends to produce more maintainable and more predictable code.
Strategies for writing more functional code include:
...
- Preferring functionals, also called
  [higher-order functions](https://en.wikipedia.org/wiki/Higher-order_function),
  like `map()`, `filter()`, and `reduce()` over loops or list
  comprehensions, as they tend to be more predictable and easier to read
  and write. A major benefit to writing more functional code is that it
  is much easier to create unit [tests](tests.qmd) for it and it is
  easier for the user to understand what the code does.
...
not enforce type hints when the code is run, as it is
[dynamically type-checked](https://en.wikipedia.org/wiki/Python_(programming_language)).
...
the `internals/` folder (as described in
[file and folder structure](#file-structure) above).
