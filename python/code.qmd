...
guide to some common and recommended practices for writing Python
code, including lessons we've learned.
...
should start by reading the PEP 8 guide to learn some basics on the style of
Python code.

However, it doesn't cover everything. For additional style guidelines,
there are several other guides you can refer to. The one we
...
Which will run Ruff and format your Python code. Then, to check if your code
...
Before knowing where to write code, in terms of how to structure your functionality in files and folders, you need to know in general what
...
1.  An individual public-facing functionality has its own Python file.
    ...
    file, but if the functionality is similar enough, multiple functions or classes
    could be grouped together in a single file.
2.  If some internal functionality is only used by public-facing
    functionality located in a single file, keep the internal functionality in
    that same file.
...
file in the `internals/` folder. By naming the file after the action, you can add other internal functions with the same action to this file as well. This keeps the internal code
...
-   Using clear inputs and one type of output for functions and methods.
    ...
    section below.
-   Avoiding [side effects](#side-effects) by only outputting the result of the function
...
-   Preferring functionals like `map()`, `filter()`, and `reduce()` over
    loops or list comprehensions, as they tend to be more predictable
...
object that is the input or output by a function or method. For example, for
...
the name of the function or method, or when it performs actions without
returning a value.
...
writing public-facing functionality. The only difference is that the name of internal objects
should start with an underscore (`_`)  and if shared by multiple public-facing functions
or methods, they should go in
the `internals/` folder (as described in [where to write code](#where-to-write-code) above).

To easily export and use the functions in the `internals` module, create an `internals/__init__.py` file
and add the function or method  to it
...
used which makes it easier to know what the script it is from in the `internals/` module.
...
function, method, and class. This helps the user understand what
...
Following the [Google style](https://google.github.io/styleguide/pyguide.html#383-functions-and-methods) for functions or methods, the docstring for the `check_heart_rate()` function looks like this:
